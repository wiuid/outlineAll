...
## 2026-09-15 Codex (AI/API 多行单元格进入编辑时丢失换行修复)
- 用户报告 AI 写入的单元格在进入编辑时丢失换行，手动输入的换行正常，随后明确要求完成修复。本轮先读取本进度、适用 AGENTS.md 和 Git diff；起点为 `main` / `88c1bc03d`，工作树干净。修改前表格工作簿、存储格式、协同会话、真实测试数据库 API/revision 基线共 8 个 project-files / 76 tests 通过；首次沙箱连接测试库返回 EPERM，按既有测试授权重跑数据库用例通过。没有改动正式环境、依赖或数据库结构，没有提交或推送本轮改动。
- 根因已通过本地原生模型与测试站 Chromium 复现：Univer 0.25.1 的 `extractPureTextFromCell` 在从普通 `v` 构造编辑文档时删除 CR/LF；手动 Alt+Enter 使用 `p.body` 的原生段落，所以能够保留。旧行为中取消编辑保留原始 `v`，确认编辑则可能把压平后的文字写回。复现只使用专用测试表，没有读取或修改用户具体的 AI 表格；证据在 `/tmp/codex-table-newline-investigation/model-probe.json`、`browser-probe.json`。
- 新增 `app/utils/tableMultiline.ts`，通过 Univer 原生 `BEFORE_CELL_EDIT` 拦截器将含实际换行的普通字符串临时转换成原生 rich text 段落；在 `TableDocument.tsx` 的工作簿创建后注册，沿用既有 disposal。通过原生模型保留单元格、行及列合成的文字样式。支持 LF、CRLF、CR、连续空行、首尾换行、中文、emoji 和强制文本前缀；既有 rich text、普通标量、字面量反斜杠、公式及共享/数组公式继续走原有流程。未修改 node_modules，也没有替换 Univer 编辑器。
- 转换发生在编辑视图，覆盖已有原始值、旧版表格、API 更新及协同传入的多行值；不会在打开、选择或取消编辑时改写存储格式、revision 或协同 epoch/hash。确认编辑和保存沿用 Univer 原生逻辑；F2 后没有输入时仍保留其原生无改动行为。已经被旧行为保存成一行的内容无法推断原换行位置，需要原始内容恢复。
- 最终自动检查：新增原生引擎测试 `app/utils/tableMultiline.test.ts` 共 15 项；合并既有工作簿/格式/协同测试为 7 个 project-files / 79 tests，真实隔离测试数据库 API/revision/协同另有 2 files / 12 tests，合计 9 个 project-files / 91 tests 全部通过。TypeScript `yarn tsc --noEmit`、完整 `yarn lint`、3 个代码文件的 oxfmt 和 Git whitespace 检查通过。独立 Vite 构建成功，产物仅在 `/tmp/codex-table-newline-fix/frontend`；保留既有 sourcemap/大 chunk 提示，没有清理仓库共享构建。
- 测试站完成 11 组 Chromium 验收：API LF/CRLF、空行及首尾换行、手动 Alt+Enter、既有 rich text、保存刷新、带 revision 的 API 更新、另一窗口发来原始多行值后的编辑与协同保存，以及真实双击/Esc、F2/Ctrl+S、内容栏编辑/Ctrl+S/刷新。实际 Esc 后服务端完整表格内容和 revision 不变；内容栏保存等待新的数据库确认版本，核对段落和加粗/文字颜色保留。最终 pageerror 和意外 HTTP 错误均为 0。
- 验收表为 `https://doc.webraa.com/doc/20260915-vmxXzQx9pY`，Vite 3001 已直接提供本轮源码，刷新测试站可使用。成功结果为 `/tmp/codex-table-newline-fix/browser-result.json`、`browser-ui-result.json`、`formula-bar-saved-cell.json`，同目录保留单元格/内容栏截图。临时入口为 `/tmp/codex-doc-browser/browser.cjs table-newline-accept` 和 `table-newline-ui`；早期失败记录源于测试提前调用尚未就绪的编辑位置、对无改动 F2 的存储形态假设及未等待保存确认，已修正测试等待和断言，未据此改动产品流程。临时登录与浏览器工具未加入仓库，未输出凭据。
- 验证日志为 `/tmp/codex-table-newline-fix-{tests,db-tests,types,lint,build,browser}.log`，基线日志为 `/tmp/codex-table-newline-fix-baseline.log`、`/tmp/codex-table-newline-fix-baseline-db.log`。本轮仅完成工作区修复和 `doc.webraa.com` 测试验收，正式 kb 仍沿用上一节已发布版本。

## 2026-09-15 Codex (换行修复发布到 kb 正式站)
- 用户明确要求推送正式环境。本轮将已验收的换行修复及记录共 4 个文件提交为 `9da2c1e13d18bba2e3525f86d53b9a7931db7617`（`fix: preserve multiline table values when editing`），推送 GitHub main 并通过远端 SHA 核对；保留历史，没有强推。沿用上一节 91 项测试、TypeScript、完整 lint、格式和 11 组浏览器验收，源码未变化。
- 从该不可变 Git 归档隔离构建后端、自动提取翻译和前端，全部通过。新镜像 `webra/outline:multiline-9da2c1e13` / `sha256:165d6b0d848cf58a09a931681a494bc490dbe7d0c817869f3ed3576bc0e7c226`，基于原正式 `webra/outline:collaboration-copy-9cb79b7ec`，继承完全相同的锁定依赖。镜像内离线公式计算结果 5、10 正常；新旧迁移清单及校验和一致，正式库待迁移为 0，本次未执行迁移。
- 正式备份 `/data/outline/backups/20260915T031845Z-multiline-9da2c1e13`，目录 0700、敏感文件 0600；PostgreSQL custom 备份 1,883,520 字节、SHA256 `57cbae6e9eb45624b51df99d77ee4c8c7eab46ca12be35c28c8801e3bf51f36d`，pg_restore 清单验证通过，保存旧 Compose/环境/执行器凭据与回滚工具。旧镜像 `sha256:34b78ee364daa6450081fa77e8c20a8362c2d06162a369e4c99925c328937d4f` 保留用于回滚。
